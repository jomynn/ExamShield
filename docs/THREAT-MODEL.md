...
| ~~R-02~~ | ~~Replicate audit log append-only to S3/CloudWatch~~ | ~~High~~ — **Fixed**: `ObjectStoreAuditLogArchiveService` writes each entry as `audit/{yyyy}/{MM}/{dd}/{id}.json` to a dedicated `examshield-audit` bucket (MinIO + Object Lock in dev; S3 + Object Lock in prod) after every `AuditLogRepository.AppendAsync`. Archive failures are swallowed so they can never block the primary database write. `NullAuditLogArchiveService` used for InMemory / AzureBlob environments. 7 new unit tests. |
